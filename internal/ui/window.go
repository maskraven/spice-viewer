...
type sessionUI struct {
	...
	// cursor is the SPICE cursor-channel bridge (remote ghost overlay).
	cursor *cursorBridge

	// host is the multi-window app host (nil in tests).
	host *appHost
...
func newSessionUI(a fyne.App, surface *Surface, bind Bindings, title string, startFullscreen bool, profile spice.PerformanceProfile, cursor *cursorBridge, host *appHost) *sessionUI {
	if title == "" {
		title = "spice-viewer"
	}
	...
	// Do not SetMaster: multi-window sessions must outlive any single window.
...
func (ui *sessionUI) releaseGrab() {
	...
	// Dual-pointer UX always keeps the host arrow; force show after ungrab.
	forceHostCursorVisible()
	if ui.win != nil {
		// Drop keyboard focus so chrome buttons cannot leave a stuck Keyable path.
...
func (ui *sessionUI) enterGrab() {
	...
	// Keep local OS pointer visible while controlling the guest.
	forceHostCursorVisible()
	// Focus the Keyable pad so KeyDown/KeyUp are not dropped when a chrome
	// Button (Focusable, not Keyable) steals canvas focus. RequestFocus so
	// this window becomes the OS key window after multi-window switches.
	ui.focusKeyboardTarget(true)
	ui.refreshStatus()
}

// shouldHoldKeyboardFocus is true when guest keyboard input should own the
// canvas focus (grabbed, no Type/Keys dark dialog).
func (ui *sessionUI) shouldHoldKeyboardFocus() bool {
	if ui == nil || !ui.grab.Active() {
		return false
	}
	return ui.darkOverlay == nil
}

// focusKeyboardTarget focuses the guest mousePad when grab should own keys.
// When requestOS is true, also RequestFocus the window (enter grab / user
// activated this session). When false, only re-focus the pad inside this
// canvas so an unfocused multi-window session does not steal OS focus back.
func (ui *sessionUI) focusKeyboardTarget(requestOS bool) {
	if ui == nil || ui.win == nil || ui.pad == nil {
		return
	}
	if !ui.shouldHoldKeyboardFocus() {
		return
	}
	ui.win.Canvas().Focus(ui.pad)
	if requestOS {
		ui.win.RequestFocus()
	}
}

// scheduleKeyboardFocusReclaim re-focuses the pad after the current focus
// transition finishes (e.g. chrome Button briefly took focus). Does not
// RequestFocus, so background windows stay in the background.
func (ui *sessionUI) scheduleKeyboardFocusReclaim() {
	if ui == nil || !ui.shouldHoldKeyboardFocus() {
		return
	}
	reclaim := func() {
		if ui.shouldHoldKeyboardFocus() {
...
func newMousePad(ui *sessionUI, view *guestView) *mousePad {
	p := &mousePad{ui: ui, view: view, ghost: newRemoteGhost()}
...
func (p *mousePad) CreateRenderer() fyne.WidgetRenderer {
	// Custom layout: guest view fills the pad; ghost is absolutely positioned
	// (Stack would force the ghost to the full pad size and break hotspots).
	objs := []fyne.CanvasObject{p.view}
	if p.ghost != nil && p.ghost.img != nil {
		objs = append(objs, p.ghost.img)
	}
	return &mousePadRenderer{pad: p, objects: objs}
}

// mousePadRenderer lays out the guest view full-bleed and the remote ghost
// at guest-mapped coordinates (dual pointer mode).
...
func (r *mousePadRenderer) Layout(size fyne.Size) {
	if r.pad != nil && r.pad.view != nil {
		r.pad.view.Resize(size)
		r.pad.view.Move(fyne.NewPos(0, 0))
	}
	if r.pad != nil {
		r.pad.refreshGhost()
	}
...
func (r *mousePadRenderer) Refresh() {
	...
	if r.pad != nil {
		canvas.Refresh(r.pad)
	}
}

func (r *mousePadRenderer) Objects() []fyne.CanvasObject { return r.objects }
...
func (p *mousePad) Cursor() desktop.Cursor {
	// Always show the local OS pointer (TeamViewer-style dual cursor).
	if p.ui != nil && p.ui.cursor != nil {
		return p.ui.cursor.hostCursor()
	}
	return desktop.DefaultCursor
}

// refreshGhost paints the lagging remote cursor from the SPICE cursor channel.
func (p *mousePad) refreshGhost() {
	if p == nil || p.ghost == nil {
		return
	}
	show := false
	var (
	...
	if p.ui != nil && p.ui.cursor != nil {
		var ok bool
		img, hotX, hotY, gx, gy, ok = p.ui.cursor.ghostRGBA()
		if ok && img != nil {
			gw, gh := 0, 0
			if p.ui.surface != nil {
			...
			sz := p.Size()
			if gw > 0 && gh > 0 && sz.Width > 0 && sz.Height > 0 {
				scale, _, _ = padContainScale(sz.Width, sz.Height, gw, gh)
				px, py = guestToPad(gx, gy, sz.Width, sz.Height, gw, gh)
				b := img.Bounds()
				w, h = b.Dx(), b.Dy()
				show = true
			}
	...
	if show {
		p.ghost.img.Image = img
		p.ghost.img.Refresh()
		p.ghost.layout(px, py, hotX, hotY, scale, w, h, true)
	} else {
		p.ghost.layout(0, 0, 0, 0, 1, 0, 0, false)
	}
}
...
// Mapping matches ImageFillContain letterboxing (see padToGuest).
func (p *mousePad) sendMousePosition(ev *desktop.MouseEvent) {
	...
	x, y := padToGuest(lx, ly, sz.Width, sz.Height, gw, gh)
...
//
// Fyne only delivers canvas SetOnKeyDown when nothing is focused. If a chrome
// Button (Focusable, not Keyable) has focus, KeyDown/KeyUp are silently
// dropped. Making the guest pad Keyable and keeping it focused while grabbed
// routes physical keys to SPICE reliably (especially with multiple windows).
...
func (p *mousePad) FocusLost() {
	...
	// Window switch or chrome stole focus: drop guest keys so modifiers cannot
	// stick, then reclaim pad focus if still grabbed (without RequestFocus).
	p.ui.releaseAllKeys()
	p.ui.scheduleKeyboardFocusReclaim()
}

func (p *mousePad) TypedRune(r rune) {
	// Desktop path uses KeyDown for physical keys. Ignore TypedRune to avoid
	// double-inject (Fyne fires KeyDown then character events).
	_ = r
}

func (p *mousePad) TypedKey(ev *fyne.KeyEvent) {
	// KeyDown already handled press; TypedKey would double-inject.
	_ = ev
}

func (p *mousePad) KeyDown(ev *fyne.KeyEvent) {
	if p == nil || p.ui == nil {
		return
	}
	p.ui.onKeyDown(ev)
...
func (p *mousePad) KeyUp(ev *fyne.KeyEvent) {
	if p == nil || p.ui == nil {
		return
	}
	p.ui.onKeyUp(ev)
...
var (
	...
	_ desktop.Keyable    = (*mousePad)(nil)
	_ fyne.Focusable     = (*mousePad)(nil)
