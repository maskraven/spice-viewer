...
func TestParseArgs_PathBeforeFlags(t *testing.T) {
	// Go flag: flags after first non-flag are not parsed; path must be last
	// or flags first. Document expected behavior: flags then path.
...
func TestRun_MissingHeadless(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"file.vv"}, &stdout, &stderr)
	if code != exitUsage {
...
func TestRun_BadVVMapsUserMessage(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "bad.vv")
	// Valid INI section but wrong type → config / not spice.
	content := "[virt-viewer]\ntype=vnc\nhost=h\npassword=p\nport=1\n"
...
func TestRun_MissingFileUserMessage(t *testing.T) {
...
func TestOpenConnectionFile_DeleteIfRequested(t *testing.T) {
	...
	// Minimal valid spice .vv with delete-this-file=1 and cleartext port.
	// CA not required when only port is set.
...
func TestOpenConnectionFile_MapsFieldTooLarge(t *testing.T) {
	...
	// Password longer than MaxPasswordLen (60).
...
func TestWipeBytes(t *testing.T) {
	...
	wipeBytes(nil) // no panic
}

// ioDiscard returns a writer that discards output (avoids importing io for one call site pattern).
