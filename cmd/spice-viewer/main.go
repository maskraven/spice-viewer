...
import (
	...
	"github.com/maskraven/spice-viewer/internal/ui"
...
func main() {
	os.Exit(run(normalizeLaunchArgs(os.Args[1:]), os.Stdout, os.Stderr))
}

// normalizeLaunchArgs strips macOS LaunchServices noise so Finder / double-click
// opens work. Finder often launches GUI apps with -psn_0_XXXX, which the flag
// package treats as an unknown flag and aborts before reading the .vv path.
func normalizeLaunchArgs(args []string) []string {
	...
	for _, a := range args {
		...
		// Older Carbon-style process serial form.
...
func parseArgs(args []string, stderr io.Writer) (options, error) {
	...
	rest := fs.Args()
	var vvs, other []string
	for _, a := range rest {
		// Normalize file:// URIs (Linux DEs) and quoted paths (Windows).
		p := ui.NormalizePathArg(a)
		if p == "" {
			continue
		}
		if ui.IsConnectionFile(p) {
			vvs = append(vvs, p)
			continue
		}
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	if opts.headless {
		...
		// GUI: argv path, or macOS Finder open-document event, or native picker.
		picked, perr := ui.ResolveConnectionPath(opts.path)
		if perr != nil {
			fmt.Fprintf(stderr, "spice-viewer: %v\n", perr)
			return exitFail
		}
		if picked == "" {
			// Cancelled open dialog or no document — quiet exit.
			return exitOK
		}
		opts.path = picked
	}
