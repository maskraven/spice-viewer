...
func (ui *sessionUI) installMenus() {
	...
	fileMenu := fyne.NewMenu("File",
		fyne.NewMenuItem("Open Connection…", func() {
			if ui.host != nil {
				ui.host.OpenPicker()
				return
			}
			// Standalone window (tests): open is a no-op without host.
		}),
...
func (ui *sessionUI) showAbout() {
	dialog.ShowInformation("About",
		"spice-viewer — SPICE client (Proxmox-friendly)\n"+
			"Library: github.com/maskraven/spice-viewer\n"+
			"Display, inputs, cursor, audio, Send Keys, hotkeys.\n"+
			"Controls: top-center auto-hide bar (Pin · CAD · Ungrab · clipboard).\n"+
			"Sessions: one window per connection (File → Open, or open another .vv).\n"+
			"Pointer: local host cursor + lagging remote ghost (cursor channel).\n"+
