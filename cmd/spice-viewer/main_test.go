...
func TestRun_HeadlessNoArgsPrintsHelp(t *testing.T) {
	// GUI with no path opens a file picker; headless still requires a .vv.
	var stdout, stderr bytes.Buffer
...
func TestNormalizeLaunchArgs_StripsPSN(t *testing.T) {
	got := normalizeLaunchArgs([]string{"-psn_0_12345", "/tmp/x.vv"})
	if len(got) != 1 || got[0] != "/tmp/x.vv" {
		t.Fatalf("got %v", got)
	}
}

func TestParseArgs_PSNThenVV(t *testing.T) {
	// Simulate Finder: -psn then path (after normalize).
	var stderr bytes.Buffer
	opts, err := parseArgs(normalizeLaunchArgs([]string{"-psn_0_999", "/tmp/guest.vv"}), &stderr)
	if err != nil {
		t.Fatal(err)
	}
	if opts.path != "/tmp/guest.vv" {
		t.Fatalf("path = %q", opts.path)
	}
}
