...
// After the action runs, keyboard focus returns to the guest pad when grabbed
// so chrome Buttons (Focusable, not Keyable) cannot permanently swallow keys.
func (ui *sessionUI) chromeIconBtn(label string, icon fyne.Resource, fn func()) *widget.Button {
...
func (ui *sessionUI) installChrome() fyne.CanvasObject {
	...
	pinBtn := ui.chromeIconBtn("Pin", theme.CheckButtonIcon(), func() {
		ui.toggleChromePin()
	})

	ungrabBtn := ui.chromeIconBtn("Ungrab", theme.CancelIcon(), func() {
		ui.chromePointerEnter()
		ui.releaseGrab()
		ui.refreshStatus()
	})
	fsBtn := ui.chromeIconBtn("Full", theme.ViewFullScreenIcon(), func() {
		ui.chromePointerEnter()
		ui.toggleFullscreen()
		ui.refreshStatus()
	})
	copyBtn := ui.chromeIconBtn("Copy", theme.ContentCopyIcon(), func() {
		ui.chromePointerEnter()
		ui.copyFromGuest()
	})
	pasteBtn := ui.chromeIconBtn("Paste", theme.ContentPasteIcon(), func() {
		ui.chromePointerEnter()
		ui.pasteToGuest()
	})
	typeBtn := ui.chromeIconBtn("Type", theme.DocumentCreateIcon(), func() {
		ui.chromePointerEnter()
		ui.showTypeTextDialog()
	})

	var sendBtn *widget.Button
	sendBtn = ui.chromeIconBtn("Keys", theme.MenuDropDownIcon(), func() {
	...
	moreBtn = ui.chromeIconBtn("", theme.MoreHorizontalIcon(), func() {
...
func (ui *sessionUI) closeDarkOverlay() {
	...
	// Return keys to the guest pad after Type/Keys dialogs.
	ui.focusKeyboardTarget(false)
}
