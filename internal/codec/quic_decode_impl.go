...
func decodeQuicBitstream(r io.Reader) (image.Image, error) {
	// read header
	ctx := &internal{}
	ctx.rb = r
	ctx.readWord()
	...
	if magic != 0x43495551 { // "QUIC"
		return nil, fmt.Errorf("codec: quic bad magic")
	}
	...
	if ctx.vers != 0 {
		return nil, fmt.Errorf("codec: quic unsupported version %x", ctx.vers)
	}
	// init channels
	for i := 0; i < 4; i++ {
	...
	if stride == -1 {
		return nil, fmt.Errorf("codec: quic unsupported image type %s", ctx.typ)
	}
	...
	switch ctx.typ {
	case quicImageTypeRGB32, quicImageTypeRGB24:
		final := &image.RGBA{
			Pix:    pixbuf,
		...
		return final, nil
	default:
		return nil, fmt.Errorf("codec: quic unsupported output type %s", ctx.typ)
	}
}
...
func (ctx *internal) decode(pixbuf []byte, stride uint32) error {
	...
	default:
		return fmt.Errorf("codec: quic unsupported decode type %s", ctx.typ)
	}
	return nil
}
