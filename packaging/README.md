...
### Opening `.vv` files (all platforms)

| Launch | Behavior |
|--------|----------|
| Double-click `.vv` / “Open with” | Path (or `file://` URI on Linux) is passed on the command line; session opens (no picker). **macOS** also accepts Finder open-document Apple Events. |
| App icon / Start Menu / empty CLI | **Native** file chooser (macOS open panel, Windows common dialog, Linux zenity/kdialog when installed). |

Linux desktop file uses `Exec=spice-viewer %u` and MIME `application/x-virt-viewer`. Windows installer registers ProgID `spice-viewer.vv` → `"spice-viewer.exe" "%1"`.

On Linux, install **zenity** (or kdialog) for the empty-launch picker: `sudo apt install zenity`.
